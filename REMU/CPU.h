...
public:
    ...
    void debug(DebuggingType type, const std::string& format_str, Args&&... args) {
        switch (type) {
        case Info: {
            if (EnableDebug) {
                if constexpr (sizeof...(args) > 0) {
                    std::string formatted_str = std::vformat(format_str, std::make_format_args(args...));
                    std::cout << "[REMU::CORE] Info: " << formatted_str << std::endl;
                }
                else if (!format_str.empty()) {
                    std::cout << "[REMU::CORE] Info: " << format_str << std::endl;
                }
                else {
                    std::cout << "[REMU::CORE] Info: " << std::endl;
                }
            }
        } break;
        case Warning: {
        ...
        case Error: {
            if constexpr (sizeof...(args) > 0) {
                std::string formatted_str = std::vformat(format_str, std::make_format_args(args...));
                std::cout << "[REMU::CORE] Error: " << formatted_str << std::endl;
            }
            else if (!format_str.empty()) {
                std::cout << "[REMU::CORE] Error: " << format_str << std::endl;
            }
            else {
                std::cout << "[REMU::CORE] Error: " << std::endl;
            }
        } break;
        }
    }
